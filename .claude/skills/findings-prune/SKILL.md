...
Maintenance pass when `paper/findings.md` outgrows brief-size (~200
lines).
...
Apply. Verify `wc -l paper/findings.md ≤ 180` (ideally ≤150).
...
into `paper/proof_of_rh.tex`. Grep index — not authoritative.
...
teammates may be using the current briefing. Age alone isn't a prune
