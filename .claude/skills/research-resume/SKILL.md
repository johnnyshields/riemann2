...
description: Resume or recover an existing research team dir in place after a crash or interruption. Reuses the existing team dir, mines prior deposits, and spawns new Opus agents into new agents/<ts>-<slug>/ subdirs without creating a new team dir.
---
...
inside the existing team dir. Use `model: "opus"` for every research agent,
always. Never spawn research teammates on Sonnet or Haiku.
...
explorers + 2 verifiers. All agents receive the full `.claude/agents/_autoresearch.md`
metaprompt and the current `findings.md`; targeted agents receive only the
relevant UV entries per `Briefing rule`.

## Dispatch in place

`TeamCreate team_name: "research-resume-<existing-ts-or-slug>-<resume-ts>"`.
Spawn new named agents with `model: "opus"` on every `Agent` call. Each agent's
...
5. Keep the resumed agents alive for follow-up dialogue and redelegation.
   `TeamDelete` only at terminal condition, explicit user halt, stale long-idle
