...
`tasks/<ts>-other-script-<slug>/` with `reports/`, `scripts/`, `notes/`.
...
2. Search the repo for existing scripts: `scripts/`, `scripts/wip/`,
   `final-scripts/paper/`, `tasks/*/scripts/`. Grep with keywords; check
   docstrings.
...
`final-scripts/paper/NN_*.py` + lore file + task dir; stage by name,
cite the dir.
