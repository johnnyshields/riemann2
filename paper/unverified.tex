...
\begin{enumerate}[nosep]
    ...
    \item the missing dependency or objection,
    \item the concrete condition for promotion, and
    \item its explicit dependencies on other UV entries (as a list of
          UV-NNN IDs, or \texttt{None}).
\end{enumerate}
...
\textbf{Depends on.} UV-009 (the calibration chain uses the quantitative
toy expansion, which is currently schematic).
...
\textbf{Depends on.} UV-002 (promotion of this status summary requires the
pair-like-vs-finite-core question to be closed one way or the other).
