...
\begin{remark}[Audit: quintic pure-value trace annihilation]
\label{rem:audit-quintic-pure-value-trace}
The generic first residual theorem presently depends on the claim that trace
annihilates the quintic pure-value slice $V_{\val}^{(5)}(h)$, or at least its
image in the residual quotient. This remains the main local order-$5$
structural statement to audit if one wants the first residual layer to be fully
closed without additional hypotheses.
\end{remark}
...
second residual determinant $\Delta_7$. What remains open is therefore not
the weight-one $q^{(7)}$-coefficient of $\Delta_7$, which is reduced to the
direct $K_1$ slice by Corollary~\ref{cor:weight-one-septic-reduction}, but
the lower/mixed septic contributions to the full total pair $(u_7,v_7)$.
\end{remark}
...
slice of the right-hand side vanishes, because $v_5$ and $c$ are governed
by the cubic/quintic layers and carry no direct septic $q^{(7)}$-slice.
But the direct $q^{(7)}$-driven slice of $\Delta_7$ is generically nonzero
by hypothesis. This is a contradiction. Dividing by $c(h)$ yields
\[
...
$s\in I_2$, $t\in I_1$. Assume for contradiction that some zero
$(s_0,t_0)\in I_2\times I_1$ exists. Choose compact subintervals
\[
J_1\Subset I_1,\qquad J_2\Subset I_2
\]
such that $(s_0,t_0)\in J_2\times J_1$ and $J_2\times J_1$ is disjoint from
the boundary-near region where $K\neq 0$. Then the zero set of $K$ in the
compact set $J_2\times J_1$ is nonempty and closed, so by
Lemma~\ref{lem:compact-first-zero-reduction} there exists a zero
$(s_*,t_*)\in J_2\times J_1$ whose $s$-coordinate is minimal in the
...
Across a generic turning boundary, Proposition~\ref{prop:adjacent-slope-range-separation}
shows that the interior tangent-slope ranges of the two adjacent intervals are
disjoint. Hence no common interior tangent slope can occur. This contradicts
the existence of $(s_*,t_*)$. Therefore no such zero exists, and
\[
K(s,t)\neq 0
\qquad
(s\in I_2,\ t\in I_1).
...
The proof of Theorem~\ref{thm:adjacent-tangent-separator-sign-regularity}
invokes Lemma~\ref{lem:compact-first-zero-reduction} after choosing a compact
rectangle $J_2\times J_1\subset I_2\times I_1$ containing one zero and
avoiding the boundary-near nonzero region. What remains to be justified fully is
a genuine zero-capture statement on the whole adjacent pair, i.e. a theorem
that all zeros of the tangent-separator kernel $K(s,t)$ on $I_2\times I_1$
are captured in such a compact region under the stated turning-boundary
hypotheses. This is a proof-organizational gap in the current turning-boundary
argument, not an issue specific to the $W=0$ case.
\end{remark}
...
\begin{remark}[Audit: directional kernel orientation]
\label{rem:audit-directional-kernel-orientation}
Lemma~\ref{lem:pair-pair-gives-directional-kernel-zero} yields a compact
interior zero of one of the two directional tangent-separator kernels. What
remains to be shown for a one-sided turning-boundary barrier theorem is an
orientation principle forcing the relevant overlap endpoint to come from the
right branch, equivalently an argument selecting $K_{21}$ rather than merely
one of $K_{21},K_{12}$. The present reduction therefore moves the turning
problem from boundary-local geometry to compact directional-kernel geometry,
but does not yet identify a preferred direction.
\end{remark}
...
\begin{remark}[Audit: $W=0$ affine-inflection structure]
\label{rem:audit-w0-affine-inflection}
The current global endgame is blocked not by the turning-point boundaries
$M=0$, but by generic boundaries of type
\[
W=0,\qquad M\neq 0.
\]
For an ordinary affine inflection
\[
y=ax^3+O(x^4),\qquad a\neq 0,
\]
the tangent line from one side meets the opposite side again, so adjacent
tangent-separator sign regularity fails in general. The remaining audited
global issue is therefore whether the actual triple $(c,v_5,\Delta_7)$
satisfies additional jet constraints at $W=0$ boundaries that rule out this
generic inflection behavior, or whether a different global invariant must be
used there.

The current evidence from the septic package suggests that this obstruction may
already be decided at order $7$, once the full top new coefficient of
$\Delta_7$ is pinned down, rather than requiring a genuinely new primitive
order-$9$ invariant.
\end{remark}
...
W(h_b)=0,\qquad M(h_b)\neq 0,
\]
the cubic-inflection coefficient of the normalized planar curve is proportional
to
\[
W'(h_b).
\]
The present fixed-sector architecture does not yet prove that
\[
W'(h_b)=0
\]
on this boundary locus. Thus the current default expectation is ordinary affine
inflection behavior unless the full leading coefficient of $W'$ can be shown
to factor through additional special structure of the triple
$(c,v_5,\Delta_7)$.

By Lemma~\ref{lem:exact-W-and-Wprime-identities} and
Corollary~\ref{cor:weight-one-septic-reduction}, the highest genuinely new
jet contribution to $W'$ is already governed by the weight-one septic
coefficient $[\Delta_7]^{[1]}$, so the present obstruction is not a new
primitive order-$9$ invariant but a question about the full order-$7$
package.
\end{remark}
...
for the two-pair and minimal-core analysis; the remaining steps are now
algebraic interaction corrections and the extraction into the finite-core
endgame.
\end{remark}
...
First, under the current direct septic nonvanishing hypothesis,
Proposition~\ref{prop:no-W-flat-interval} excludes the flat case
\[
W\equiv 0
\]
on any connected open interval with $c\neq 0$ and $M\neq 0$.

Second, Theorem~\ref{thm:finite-order-w-zero-local-sterility} shows that every
finite-order isolated boundary of type
\[
W(h_b)=0,\qquad M(h_b)\neq 0
\]
is locally sterile for pair-pair birth.

Thus the remaining generic obstruction is no longer bulk $W=0$ geometry, but a
global continuation / zero-capture problem across adjacent boundaries,
especially on the turning-boundary side. By
Lemma~\ref{lem:pair-pair-gives-directional-kernel-zero}, any adjacent
pair-pair line across a generic turning boundary produces a compact interior
zero of one of the two directional tangent-separator kernels.

Moreover, the exact identities for $W$ and $W'$ still show that any residual
$W=0$ pathology is encoded by the order-$7$ package through $\Delta_7$, not by
a genuinely new primitive order-$9$ invariant.
\end{remark}
...
\]
What remains audited at the geometric level is therefore the compact
directional-kernel / zero-capture problem, together with the orientation issue
isolated in Remark~\ref{rem:audit-directional-kernel-orientation}.
