...
\begin{proof}
Under the stated hypotheses, every adjacent boundary is of turning type.
For such adjacent pairs,
Theorem~\ref{thm:adjacent-tangent-separator-sign-regularity} and
Lemma~\ref{lem:adjacent-support-ordering} apply. Hence adjacent fixed-slope
intercept sets are strictly ordered. Iterating this ordering over the
$x=v_5/c$ ordering of ECT intervals shows that no affine line can be
$2$-secant to two distinct ECT intervals. Therefore no global four-secant
line exists on the block.
\end{proof}
...
The remaining geometric obstruction is now concentrated at boundaries of type
\[
W(h_b)=0,\qquad M(h_b)\neq 0.
\]
At such points, the normalized planar curve
\[
\Gamma(h)=\left(\frac{v_5(h)}{c(h)},\frac{\Delta_7(h)}{c(h)}\right)
\]
has cubic coefficient proportional to $W'(h_b)$. The current architecture
does not yet force $W'(h_b)=0$, so the default local model is an ordinary
affine inflection. Thus the turning-boundary part of the secant/tangent
geometry is comparatively well controlled, while the real remaining issue is to
find a $W=0$-specific replacement theorem or an additional structural
identity forcing nongeneric affine-jet behavior there.

Moreover, the exact identities for $W$ and $W'$ show that the highest
genuinely new jet contribution to the $W=0$ obstruction is already governed by
the septic residual determinant $\Delta_7$; thus the present issue is not the
appearance of a fundamentally new order-$9$ invariant, but the full closure of
the order-$7$ package at $W=0$.
\end{remark}
...
$(c,v_5,\Delta_7)$ is an ECT system. The turning-boundary case
\[
M=0,\qquad M'\neq 0,\qquad N\neq 0
\]
is comparatively well behaved. The remaining nonlocal geometric issue is the
$W=0,\ M\neq 0$ case, where the cubic-inflection coefficient is controlled by
$W'(h_b)$ and the current architecture does not yet force nongeneric
vanishing. The present evidence suggests that this $W=0$ obstruction is still
encoded by the full septic coefficient of $\Delta_7$, via the exact identities
for $W$ and $W'$, rather than by a genuinely new primitive order-$9$ scalar.
The remaining audited issues are therefore both local and global:
first-order surjectivity, the quintic pure-value quotient detector, the
completion of the order-$7$ fixed-sector algebra, and the special
$W=0$-boundary structure of the actual triple $(c,v_5,\Delta_7)$.
\end{remark}
