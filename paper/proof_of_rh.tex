...
Let $J_3^{\pair}(h)$ denote the cubic jet of the normalized one-pair family.
Then the cubic value-family $V_{\val}^{(3)}$ is confined to the off-diagonal
sector. Moreover, the diagonal part of the actual cubic jet is
\[
...
\end{pmatrix},
\]
where $x_1\neq x_2$ are the diagonal entries of $X_0=G_0^{-1/2}$,
$v(h)$ is the off-diagonal entry of $X_1(h)$, and
\[
B(h)=\frac{q(h)^2q'(h)}{4}-\frac{q'''(h)}{48}.
\]
In particular, if
\[
v(h)\not\equiv 0,\qquad B(h)\not\equiv 0,\qquad x_1\neq x_2,
\]
then
\[
[J_3^{\pair}(h)]\neq 0
...
On the generic one-pair region, define the cubic leading line
\[
...
J_{5,K_1}
=
-\,X_4K_1X_0+X_3K_1X_1-X_2K_1X_2+X_1K_1X_3-X_0K_1X_4.
\]
Then
\[
...
other quintic contribution to $\operatorname{tr}J_5^{\pair}(h)$. Consequently
\[
...
\label{rem:wip-no-cancellation-quintic-trace}
This lemma is presently a bookkeeping target rather than a fully closed
proof. The intended argument isolates the monomial
$\kappa(h)\eta_2(h)q^{(5)}(h)$ inside $\operatorname{tr}J_5^{\pair}(h)$ and
shows that no other quintic contribution produces the same monomial with
opposite coefficient. For drafting purposes, this lemma should be read as
the main remaining local technical bottleneck.
\end{remark}

\begin{proposition}[Nontrivial first residual class]
\label{prop:first-residual-class-nontrivial}
Assume the generic nondegeneracy hypotheses of
Proposition~\ref{prop:cubic-leading-order} and the no-cancellation statement
of Lemma~\ref{lem:no-cancellation-quintic-trace}.
Then the first residual class is nonzero:
\[
\overline v_1(h)\neq 0
\quad\text{in }Q/\ell_h
\]
on the generic one-pair region.
Equivalently, the quintic quotient coefficient is not collinear with the
cubic leading line:
\[
v_1(h)\notin \ell_h.
\]
...
\begin{proof}
By Proposition~\ref{prop:cubic-leading-order}, the cubic quotient coefficient
\[
v_0(h)=[J_3^{\pair}(h)]
\]
is nonzero and defines the leading line $\ell_h$.
By Lemma~\ref{lem:no-cancellation-quintic-trace},
\[
\operatorname{tr}J_5^{\pair}(h)\not\equiv 0.
\]
But the cubic leading line is diagonal-traceless, whereas a nonzero trace
component is automatically transverse to that line. Therefore the quintic
quotient coefficient $v_1(h)$ cannot lie in $\ell_h$, so its image in
$Q/\ell_h$ is nonzero:
\[
\overline v_1(h)\neq 0.
\]
Thus the residual germ is nontrivial at the first step, and the one-pair
quotient odd germ cannot be projectively constant.
\end{proof}

\begin{remark}[Work in progress]
\label{rem:wip-first-residual-class-nontrivial}
This proposition is conditional on the no-cancellation mechanism in
Lemma~\ref{lem:no-cancellation-quintic-trace}. Structurally, the argument is
in place: cubic order supplies the generic leading line, and quintic trace
supplies the first residual witness. The remaining task is to make the
quintic bookkeeping completely explicit.
\end{remark}
...
Proposition~\ref{prop:first-residual-class-nontrivial}, on the generic
