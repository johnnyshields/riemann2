...
\begin{remark}[Work in progress: planar ECT and tangent-separator reformulation]
\label{rem:wip-planar-convexity-reformulation}
The later working analysis suggests that the remaining generic obstruction
should be reformulated in terms of the normalized planar curve
\[
\Gamma(h)=\left(\frac{v_5(h)}{c(h)},\frac{\Delta_7(h)}{c(h)}\right)
\]
and the tangent-separator kernel
\[
K(s,t)=\det(F(s),F'(s),F(t)),
\qquad
F(h)=(c(h),v_5(h),\Delta_7(h)).
...
c(h)\neq 0,\qquad M(h)\neq 0,\qquad W(h)\neq 0,
\]
the triple $(c,v_5,\Delta_7)$ is an ECT system, so every affine line meets
the corresponding branch of $\Gamma$ in at most two points. The remaining
global obstruction is therefore not same-branch secancy, but the possibility
that tangent or secant lines interact across adjacent ECT intervals.
\end{remark}
