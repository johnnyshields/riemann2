...
Lemma~\ref{lem:pair-pair-gives-directional-kernel-zero} reduces an adjacent
pair-pair line across a generic turning boundary to a compact interior zero of
one of the two directional tangent-separator kernels. With
Theorem~\ref{thm:conditional-symmetric-directional-kernel-exclusion}, the
orientation issue is no longer the main gap. What remains is the symmetric
zero-capture problem for $K_{21}$ on $I_2\times I_1$ and $K_{12}$ on
$I_1\times I_2$.
\end{remark}
...
symmetric zero-capture / continuation problem for the two directional
tangent-separator kernels across adjacent turning boundaries. By
...
zero of one of the two directional tangent-separator kernels, and by
Theorem~\ref{thm:conditional-symmetric-directional-kernel-exclusion} the
remaining issue is not orientation selection between them, but proving
zero-capture in both directions.
