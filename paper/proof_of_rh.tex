...
\end{equation}
where the perturbation is taken along the pure value direction
$q_1=q_2=B_\zeta(m)+\alpha$, with the local-model derivative and curvature
data frozen at $q'=q''=0$.
In the fixed-scaled regime, $A_{\val}(m,\sigma)$ depends on $(m,\sigma)$ only
through $x=B_\zeta(m)\sigma$; we write $A_{\val}(x)$ for
$A_{\val}(m,\sigma)$ in this variable.

\begin{lemma}[Explicit value-channel matrix]\label{lem:Aval-explicit}
Write $B:=B_\zeta(m)$. Then
...
\end{pmatrix},
\end{equation}
and the off-diagonal entries are exact negatives of each other:
$A_{\val,21}(x)=-A_{\val,12}(x)$.
Consequently,
\begin{equation}\label{eq:A-val-norm}
\|A_{\val}(x)\|_\F\asymp \frac{x}{B_\zeta(m)}
\qquad (x\to 0,\ x>0).
\end{equation}
\end{lemma}

\begin{proof}
In the local model $q_1=q_2=q:=B+\alpha$ with $q'=q''=0$, the same-point Gram
block of \eqref{eq:J-T} reduces to a diagonal matrix:
\[
\pi J(T_j)=
\begin{pmatrix} 2q & 0 \\ 0 & q^3/6 \end{pmatrix},
\qquad
(\pi J(T_j))^{-1/2}=
\begin{pmatrix} (2q)^{-1/2} & 0 \\ 0 & \sqrt 6\,q^{-3/2} \end{pmatrix}.
\]
Symmetric placement gives $s=T_1-T_2=-\sigma$ and
$\Phi(T_1)-\Phi(T_2)=-q\sigma$, so in the notation of \eqref{eq:N12},
$\Delta=-y$ with $y:=q\sigma$. Substituting into \eqref{eq:N12} and applying
the whitening with $M_1^{-1/2}=M_2^{-1/2}$ gives
\[
\Omega_\infty(q,\sigma)=F(y),
\qquad
F(y):=
\begin{pmatrix}
\dfrac{\sin y}{y} & \dfrac{\sqrt3(y\cos y-\sin y)}{y^2}\\[1.5ex]
-\dfrac{\sqrt3(y\cos y-\sin y)}{y^2} & \dfrac{3\bigl(2y\cos y+(y^2-2)\sin y\bigr)}{y^3}
\end{pmatrix}.
\]
The dependence on $(q,\sigma)$ enters only through $y=q\sigma$. Since
$\partial_\alpha y|_{\alpha=0}=\sigma$ and $\sigma=x/B$,
\[
A_{\val}(m,\sigma)=\partial_\alpha \Omega_\infty\big|_{\alpha=0}
=\sigma\,F'(x)
=\frac{x}{B}\,F'(x).
\]
A direct small-$y$ expansion gives
\[
F'_{11}(y)=-\tfrac{y}{3}+O(y^3),
\qquad
F'_{12}(y)=-\tfrac{\sqrt3}{3}+O(y^2),
\qquad
F'_{22}(y)=-\tfrac{3y}{5}+O(y^3),
\]
and $F_{21}(y)=-F_{12}(y)$ exactly (from $q_1=q_2$, which forces
$N_{12,21}=-N_{12,12}$ after symmetric whitening). Multiplying by $x/B$
yields the displayed matrix entries and the exact identity
$A_{\val,21}(x)=-A_{\val,12}(x)$. The norm estimate follows from the
off-diagonal leading term $\sqrt3 x/(3B)$.
\end{proof}

\begin{proposition}[Transverse vanishing of the value channel]
\label{prop:PhiK-Aval-zero}
For all $(m,\sigma)$ in the symmetric local regime,
\[
\Phi_K\bigl(A_{\val}(m,\sigma)\bigr)=0.
\]
\end{proposition}

\begin{proof}
By Lemma~\ref{lem:Aval-explicit}, $A_{\val,21}=-A_{\val,12}$, hence
$\Phi_K(A_{\val})=A_{\val,12}+A_{\val,21}=0$.
\end{proof}

\begin{lemma}[Nondegenerate pairing with the toy anomaly]\label{lem:pairing}
For each compact interval $D\subset(0,\infty)$ there exists $x_0=x_0(D)>0$
such that
\begin{equation}\label{eq:pairing-nondegenerate}
|\langle A_{\val}(x_0),M(d)\rangle_\F|\asymp \frac{x_0}{B_\zeta(m)}
\qquad(d\in D).
\end{equation}
\end{lemma}

\begin{proof}
By Lemma~\ref{lem:Aval-explicit}, $A_{\val}(x)=(1/B_\zeta(m))(xP+O(x^2))$,
where $P$ is the rank-two matrix whose nonzero entries are the off-diagonal
leading coefficients $\mp\sqrt3/3$. Hence
\[
B_\zeta(m)\,\langle A_{\val}(x),M(d)\rangle_\F
=
x\,\langle P,M(d)\rangle_\F + O(x^2).
\]
The pairing $\langle P,M(d)\rangle_\F$ is a rational function of $d$ arising
from the off-diagonal entries of $M(d)$; it does not vanish on any compact
$D\subset(0,\infty)$. Choosing $x_0=x_0(D)>0$ small enough that the
$O(x^2)$ term is dominated gives the stated asymptotic.
\end{proof}

This suggests the correct calibration functional.

\begin{definition}[Calibrated value functional]
Fix a compact interval $D\subset(0,\infty)$ and a sufficiently small
$x_0=x_0(D)>0$ as in Lemma~\ref{lem:pairing}. Define
...
\begin{proposition}[Canonical calibration theorem]\label{prop:canonical-calibration}
...
\[
\langle A_{\val},M(d)\rangle_\F \asymp \frac{x_0}{B_\zeta(m)}
\]
and
\[
\|A_{\val}\|_\F^2 \asymp \left(\frac{x_0}{B_\zeta(m)}\right)^2,
\]
we have
\[
\Psi_{x_0,d}(A_{\val})
=
\frac{\|A_{\val}\|_\F^2}{\langle A_{\val},M(d)\rangle_\F}
=
...
\]
\item the baseline blocks satisfy the local variation estimate
\[
\sup_{|s|<\rho_0/Q}
...
\rho_0Q+o(Q).
\]
For $\rho_0$ sufficiently small and $m$ sufficiently large, this is less than
half the baseline spectral gap from hypothesis (2). Therefore
\[
...
\begin{proof}
By Proposition~\ref{prop:odd-expansion}, $H_m(s)$ is odd and holomorphic on
$|s|<\rho_0/Q$ and admits the expansion
\[
H_m(z/Q^2)
=
\sum_{r\ge 0} c_{2r+1}(m)\frac{z^{2r+1}}{Q^{2r+4}},
...
\]
Applying the odd-moment cancellation with the coefficients $a_j^{(N)}$ kills
the first $N-1$ odd terms; the surviving moment factor is exponentially tame
for $N=\alpha Q$, which gives the stated bound.
\end{proof}
