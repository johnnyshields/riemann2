...
1. Exact fixed-shear / quotient-diagonal package theorem on `\{S=0\}\cap\{K_v=0\}`.
2. Corrected edge-law theorem on good patches `J\Subset\{c\neq 0, M\neq 0\}`.
3. Reduced-`\widehat\Psi` coincidence / package-level coincidence theorem.
4. Hidden extraction theorem from package data to the first surviving odd coefficient of `H_m` / first nonzero `\Xi_{\zeta,\le H}^{(N)}`.
