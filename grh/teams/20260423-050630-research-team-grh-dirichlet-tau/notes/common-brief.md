...
Explore how the current RH draft architecture might extend to primitive
Dirichlet \(L\)-functions, broader GRH-style settings, and the Ramanujan
\(\tau\) \(L\)-function. The deliverable is not a proof of GRH. The goal is
to identify:

- what is genuinely zeta-specific in the draft;
- what looks portable to completed \(L\)-functions;
- what breaks for complex characters, higher degree, or non-self-dual cases;
- what insights can be ported back to `paper/proof_of_rh.tex`.

## Writing discipline

- Separate three things: proved / conditional on [X] / missing.
- State facts directly. No hype.
- Reduce gaps to concrete missing sub-statements.
- Qualify impossibility claims with "from [scope] alone." 
- Label candidate syntheses with `[confirmed]`, `[conditional]`, or `[candidate]` before merging.
- End with an honest verdict.
- `unsupported`, `blocked`, and `no progress` are acceptable returns.

## Report schema

Every report must contain exactly these seven fields:
...
Allowed `Status` values: `proved | computational | heuristic | open | rejected`.
...
- Do not try to prove GRH outright.
- Do not assume the current RH draft is fully closed; respect the pair-like / finite-core and high-height caveats.
- Do not spend time on bibliography cleanup or unrelated chat-archive work.
- Do not promote anything into the main paper.
