...
- protected surfaces the agent must not edit (`<main>.tex`, team-dir state files, `CLAUDE.md`, `lore/`, other agents' dirs — unless the role grants exception);
...
Researchers do **not** write to `<main>.tex`, team-dir state files, `CLAUDE.md`, `lore/`, or another agent's dir — except `fix` mode (Phase-1 referee), which may edit the paper within the scoped region in its brief.
...
Skills are pointers, not recipes. Name the goal, the invariants, and the outputs; trust the model with the path. Refer to `CLAUDE.md` sections instead of repeating them — `follow Writing discipline` beats re-listing the bullets. Bash snippets only when the exact command is load-bearing. Anti-pattern lists only for things that would reasonably be tried and shouldn't be. Keep skills under ~80 lines.
...
Every section in `rh_rebuild.tex` carries five status indicators under its heading, one per axis, each as its own macro so axes can be edited independently:
