...
- **Every asymptotic has rate checks.** A claimed `O(h^k)` is fitted on a log–log grid and the slope asserted near `k`; a claimed `f(t) = g(t) + O(t^{-k})` is checked by `(f - g) * t^k → const` over a height ladder with the residual bounded.
- **Every scope limitation is stress-tested.** A hypothesis like `h < |s|/3` or "fixed separation" gets a sweep that crosses the boundary and demonstrates degradation, so the constraint is shown to be load-bearing rather than ornamental.
- **Independent routes converge.** Where a quantity admits two computation paths (closed formula vs direct conjugation; trace/det vs explicit eigenvalues; symbolic Taylor vs numerical finite difference), both are computed and compared.
- **Every print is an assertion.** Diagnostic output is for the reader; the script's exit status is for CI.  Convert "print-only" diagnostics into `assert` with explicit tolerance whenever the displayed quantity is supposed to obey a law.

These standards apply equally to sympy and simulation scripts; sympy verifies the algebraic identity, simulation verifies the numerical regime where the identity is to be applied, and the two together close the loop.  A simulation does not replace a proof, but for every computational claim in `rh_rebuild.tex` the harness should produce a hard failure if the claim breaks.
