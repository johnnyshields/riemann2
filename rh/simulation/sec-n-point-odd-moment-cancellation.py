...
Coverage:
  ...
  * Coefficient identities (lem:n-point-coefficients) at high N
    (N = 1 through 12), beyond what symbolic verification reaches in
    reasonable time.
  ...
  * Convolution stability: random even bumps (mpmath-generated)
    ...
    asserted to vanish to the stated high-precision tolerance.
  * Stress: confirm the FIRST surviving odd derivative
    d^{2N-1} mu_hat_{N,Q}(0) is nonzero in absolute
    value, including the N=1 edge case.
...
def check_mu_hat_odd_vanishing(N_values=(1, 2, 3, 4, 5, 6, 7)):
    print("=" * 70)
...
def check_convolution_parity(N_values=(1, 2, 3, 4, 5), n_bumps=5):
    print("=" * 70)
    ...
    print("         product with mu_hat_{N,Q}, confirmed to the asserted high-precision tolerance.")
...
def check_ell1_growth(N_values=(1, 2, 5, 10, 20, 30, 50)):
    print("=" * 70)
...
def check_difference_identity_numerical(N_values=(1, 2, 3, 4, 5, 6, 8)):
...
def main():
    ...
    check_coefficient_identities(N_values=(1, 2, 3, 4, 5, 6, 7, 8, 10, 12))
    print()
