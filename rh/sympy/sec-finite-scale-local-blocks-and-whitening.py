...
def verify_signed_s_symmetry():
    ...
    sp.pprint(diff_sym)
    assert diff_sym == zeros(2, 2), (
        f"signed-s transpose identity failed: {diff_sym}"
    )
    print()
...
# ---------------------------------------------------------------------------
# (iv) q_± Taylor expansion sign pattern (proof regression guard).
# ---------------------------------------------------------------------------
