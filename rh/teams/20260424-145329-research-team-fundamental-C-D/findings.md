...
- **UV-026 mixed-input parity:** `M(-z)=M(z)^T`; diagonal entries are even, and `[z^5]M` lives only in the off-diagonal antisymmetric channel from even source derivatives `r^{(2)},r^{(4)},r^{(6)}`. Provenance: `agents/20260425-002114-gap-closer-UV026-mixed-parity-theorem/report.md` through `agents/20260425-010000-verifier-adversarial-UV026-L2YR0-order/report.md`. Under the clean homogeneous scalar grade `Gr_a r=r^{(a+2)}`, all `Y`-slot cubic families (`L_1YR_1`, `L_2YR_0`, `L_0YR_2`) are verified absent at `B_7`; nonzero alternatives require a separate matrix-order/no-double-counting theorem and reopen those gates.
