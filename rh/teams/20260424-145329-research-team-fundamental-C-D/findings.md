...
- **UV-026 mixed-input parity:** `M(-z)=M(z)^T`; diagonal entries are even, and `[z^5]M` lives only in the off-diagonal antisymmetric channel from even source derivatives `r^{(2)},r^{(4)},r^{(6)}`. Provenance: `agents/20260425-002114-gap-closer-UV026-mixed-parity-theorem/report.md` and `agents/20260425-003100-verifier-adversarial-UV026-parity-grade-conflicts/report.md`. `M_i^{[5]}` is therefore a conditional placeholder; moving those even derivatives into scalar grade five is rejected from current source-grade bookkeeping unless a new matrix-order or no-double-counting theorem is stated.
