...
- **UV-026 mixed-input parity:** `M(-z)=M(z)^T`; diagonal entries are even, and `[z^5]M` lives only in the off-diagonal antisymmetric channel from even source derivatives `r^{(2)},r^{(4)},r^{(6)}`. Provenance: `agents/20260425-002114-gap-closer-UV026-mixed-parity-theorem/report.md`, `agents/20260425-003100-verifier-adversarial-UV026-parity-grade-conflicts/report.md`, and `agents/20260425-003100-gap-closer-UV026-parity-corrected-grade/report.md`. `M_i^{[5]}` is therefore a conditional placeholder; under the clean homogeneous scalar grade `Gr_a r=r^{(a+2)}` the `L_1YR_1` middle coefficient is absent at `B_7`, and nonzero alternatives require a matrix-order or no-double-counting theorem.
