...
%     q''^{(2)} ~ q_0^3, so the resulting F_toy(d) depends only on
%     d = q_0 s and not separately on q_0.  This is what gives the
%     visibility exponent A_toy = 0 (no Q-dependence in the lower
%     bound).
%
...
Fix a compact interval \(D=[d_-,d_+]\subset(0,\infty)\) and
\(Q_0\ge 1\).  There exist \(u_G=u_G(D,Q_0)\in(0,\tfrac12]\) and
\(c_G(D,Q_0)>0\) such that whenever \(d=q_0\,s\in D\),
\(|u|\le u_G\), and \(q_0\ge Q_0\),
\[
...
\(\Tr G_{\toy,\pm}\le C(D,u_G)q_0^3\).  Therefore
\[
...
\ge c(D,u_G)Q_0.
\]
Absorbing the last quantity into \(c_G(D,Q_0)\) gives the claim.
\end{proof}
...
real-analytic function of \(u^2\) by \eqref{eq:toy-phase-derivatives}
and \eqref{eq:Gpm}--\eqref{eq:Nm}.  By
Lemma~\ref{lem:toy-block-positivity},
\(G_{\toy,\pm}(s;m,u)^{-1/2}\) is real-analytic in \(u^2\) on the
parameter region (real-analyticity of the SPD inverse-square-root map on the positive-definite cone).
Hence \(\widehat\Om_\toy(s;m,u)\) and the scalar
\(\mathcal A_\toy(\widehat\Om_\toy(s;m,u))\) are real-analytic in
\(u^2\), with \(u^0\) coefficient zero by
Lemma~\ref{lem:A-toy-baseline-vanishing}.  The leading nonzero term
is at order \(u^2\); call its coefficient \(F_\toy(d,q_0)\) after
substituting \(s=d/q_0\).
...
\eqref{eq:F-toy-lower-bound}.  The remainder \(R_\toy\) is the \(u^4\)-and-higher coefficient of the
real-analytic expansion.  The same explicit formulas used to compute
\(F_\toy\), together with Lemma~\ref{lem:toy-block-positivity}, show that
these coefficients are uniformly bounded for \(d\in D\), \(|u|\le u_1\),
and \(q_0\ge Q_1(D)\), after decreasing \(u_1\) and increasing
\(Q_1(D)\) if necessary.
\end{proof}
...
\begin{proof}
Choose \(Q_\toy(D)\ge Q_1(D)\).  Apply Lemma~\ref{lem:toy-block-positivity}
with \(Q_0=Q_\toy(D)\), and let \(u_G(D,Q_\toy(D))\) be its smallness
threshold.  Choose
\[
        u_\toy(D)\le \min\{u_1(D),u_G(D,Q_\toy(D))\}
\]
