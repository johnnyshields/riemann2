...
cross-block jets of Section~\ref{sec:jet-limit-local-blocks} extend to
two same-point Gram blocks \(G_{m,\pm}(s)\) and a mixed block
\(N_m(s)\); whitening these into the dimensionless
\(\widehat\Om_\z(s;m)\)
...
Fix \((m,s)\in\mathcal D_T^\times\) and write
\[
        t_\pm = m\pm s/2,
        \qquad q_\pm = q(t_\pm),
...
If \(\Ph\in C^3\) on a neighborhood of \(m\), the entries of
...
\(G_{m,\pm}(s)^{-1/2}\) denote the positive definite inverse square
root and set
\begin{equation}
