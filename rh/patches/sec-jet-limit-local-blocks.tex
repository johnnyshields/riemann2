...
\begin{proof}
By Lemma~\ref{lem:phase-derivative-lower-bound},
\(q(T)\ge Q^{-c_q}\) on retained packets with absolute \(c_q=0\) at
sufficiently large \(T\); standard derivative estimates on the
Riemann--Siegel theta function give \(|q'(T)|,|q''(T)|\le Q^{C_d}\)
for an absolute exponent \(C_d\) (in fact \(q'(t)=O(t^{-1})\) and
\(q''(t)=O(t^{-2})\), so \(C_d\) can be taken \(0\)).
...
positivity of \(J(T)\) to the polynomial lower bound (P2) of
Definition~\ref{def:local-phase-chart}, which is established
unconditionally in
Lemma~\ref{lem:phase-derivative-lower-bound} (\(c_q = 0\) at
sufficiently large \(T\)).  The trace/determinant computation and
the \(\det/\Tr\) lower bound are pure linear algebra and verified
in \texttt{rh/sympy/sec-jet-limit-local-blocks.py}; thus
Lemma~\ref{lem:same-point-gram-positivity} closes unconditionally
once \S\ref{sec:local-kernel-and-jet-normalization} migrates.
\end{remark}
