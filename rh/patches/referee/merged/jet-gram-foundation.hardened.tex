...
\label{eq:theta-definition}
 ...
 := \Im L\!\left(\frac14+\frac{it}{2}\right)
    -\frac{t}{2}\log\pi.
\end{equation}
This is the standard Riemann--Siegel phase.  With
\(Z(t):=e^{i\theta(t)}\zeta(1/2+it)\), the functional equation gives
\[
...
\theta(t)=\frac{t}{2}\log\frac{t}{2\pi}-\frac{t}{2}-\frac{\pi}{8}
          +\frac{1}{48 t}+O(t^{-3}),
\]
again with the displayed remainder differentiable twice with the
corresponding differentiated bounds.  Differentiating gives
...
\begin{proof}
Set \(x=T+u\), \(y=T+v\), and write \(r=|u|+|v|\).  All derivatives
below are evaluated at \(T\).  To avoid dividing an unfactored remainder
by \(u-v\), first factor the phase difference:
\[
\Ph(T+u)-\Ph(T+v)=(u-v)A(u,v),
\]
where, for \(u\ne v\),
\[
A(u,v)=\int_0^1 q\bigl(T+v+\tau(u-v)\bigr)\,d\tau,
\]
and the same formula extends continuously to \(u=v\).  Taylor expansion
of the integrand gives
\[
A(u,v)
= q+\frac{q'}2(u+v)+\frac{q''}{6}(u^2+uv+v^2)+O(r^3).
\]
Since \(\Ph(T+u)-\Ph(T+v)=(u-v)A(u,v)=O(r)\),
\[
...
constant may depend on \(|s|^{-1}\) and on local \(C^5\)-bounds for
\(\Ph\), but not on \(h\).  Here
\begin{equation}
\label{eq:cross-N12}
N_{12}=
\begin{pmatrix}
...
\begin{proof}
Since \(T_1 \ne T_2\), the kernel is smooth near \((T_1, T_2)\).  The
condition \(h<|s|/3\) ensures
\(|s+u_1-u_2|\ge |s|/3\) for \(|u_1|,|u_2|\le h\), so the Taylor
remainder is controlled on the sampled square.  Write
