...
%   - Final hardening pass: theta remainder differentiability, diagonal
%     rescaling terminology, off-diagonal neighborhood control, and
%     fixed-separation scope made explicit.
%
% Closes:
...
the corresponding differentiated bounds.  Differentiating gives
...
\]
We write \(\Ph\) and \(q=\Ph'\) when \(T\) is fixed by context.  This is a
specialized theta chart, not a general phase-chart axiom.  The chart
...
\]
Here the subscript indicates that the constant may depend on a finite
power of \(|s|^{-1}\) and on the chosen off-diagonal neighborhoods.
