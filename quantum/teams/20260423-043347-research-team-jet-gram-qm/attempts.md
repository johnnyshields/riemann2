...
| `20260424-192133-gap-natural-protocol` | UV-014 | Test whether principal-angle/operator data has a natural two-projector/reflection measurement protocol beyond the thin yes/yes test, or only existing reformulations. | terminal | `agents/20260424-192133-gap-natural-protocol/report.md` gives a positive two-reflection protocol: phase estimation on `(2Pi_- - I)(2Pi_+ - I)` returns phases `±2 theta_i` on principal two-planes. | Promoted to `quantum/paper/jet_gram_quantum_note.md`; UV-014 removed from `uv.md`. | This is a coherent protocol for estimating existing principal-angle data, not a new invariant. |
...
| `20260424-193527-gap-nonbenchmark-genericity` | UV-015 | Prove a structural genericity theorem for richer-than-overlap behavior, or reduce it to a sharp transversality/rank condition. | keep pending verification | `agents/20260424-193527-gap-nonbenchmark-genericity/report.md` proves a finite endpoint-2-jet genericity theorem for first-order `O_1` line-angle data; script output verifies seed `C=1/2`, `Delta=1/4*sqrt(2)`. | Send to adversarial verifier before promotion or UV removal. | This is the final live UV in the quantum ledger. |
| `20260424-194246-verifier-nonbenchmark-genericity` | UV-015 | Adversarially test the endpoint 2-jet determinant theorem, density/interpolation step, and scope caveats. | running | Briefed against `agents/20260424-193527-gap-nonbenchmark-genericity/report.md` and its determinant script. | Await report; accept, revise, or keep UV-015 open. | Verification required before final quantum UV proof-state change. |
...
- **Current best:** The safe canonical core is the subspace/operator package:
  ...
  alone. Under coherent reflection access, the principal angles also have a
  phase-estimation protocol via `(2Pi_- - I)(2Pi_+ - I)`.
...
- **Next:** Per user redirection on 2026-04-24, do not default to a 3+3+2
  roster. Attack UV-015 (nonbenchmark genericity), the last live UV in this
  quantum ledger.
