...
| `20260424-191454-gap-Ar-Or-role-split` | UV-013 | Decide whether `O_r` versus `A_r` has a role criterion or only a no-global-winner theorem. | terminal pending verification | `agents/20260424-191454-gap-Ar-Or-role-split/report.md` gives a role criterion: `O_r` for horizontal/value-channel-removed geometry, `A_r` for ambient state-line endpoint packages; no global winner is forced. | Send to adversarial verifier before promotion or UV removal. | This is the next structural target after closing matrix/frame finality. |
| `20260424-191817-verifier-Ar-Or-role-split` | UV-013 | Adversarially test the `O_r`/`A_r` role criterion and no-global-winner theorem. | running | Briefed against `agents/20260424-191454-gap-Ar-Or-role-split/report.md` and prior decomposition/quotient/benchmark reports. | Await report; accept, revise, or keep UV-013 open. | Verification required before changing UV-013 proof state. |
...
- **Next:** Per user redirection on 2026-04-24, do not default to a 3+3+2
  roster. Attack UV-013 (`O_r` versus `A_r`) next.
