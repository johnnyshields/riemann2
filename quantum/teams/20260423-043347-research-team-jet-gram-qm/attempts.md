...
| `20260424-191454-gap-Ar-Or-role-split` | UV-013 | Decide whether `O_r` versus `A_r` has a role criterion or only a no-global-winner theorem. | running | Follow-on dispatch in `dispatch.md`; prior reports establish `A_r=span{psi}\oplus O_r`, quotient, and benchmarks where `O_r` is sometimes finer. | Await report; if terminal, send to verifier before promotion or UV removal. | This is the next structural target after closing matrix/frame finality. |
