...
- In that multiparameter covariant-jet hierarchy, ordering matters only up to
  ...
  covariant jets.
- The multiparameter covariant and symmetrized jet hierarchies do not determine
  a canonical ordered matrix/frame package from the ray-field data alone. The
  exact surviving basis-free data is the `O_r`/`A_r` filtration and its
  projector, cross-contraction, polar, and principal-angle/operator package;
  matrices require extra frame-selecting structure or are merely biunitary-orbit
  representatives.
