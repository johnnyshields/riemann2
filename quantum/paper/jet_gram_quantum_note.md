...
5. Higher raw jet matrices fail, but the higher osculating subspaces survive.
   In a fixed ambient Hilbert space, their principal angles and projector
   compressions are canonical transport-free invariants.
6. There are now explicit benchmark families on both the ambient `A_r` side and
...
1. Define a higher-order covariant-jet hierarchy if a transport-based matrix
   language is still desired beyond the ambient subspace picture.
